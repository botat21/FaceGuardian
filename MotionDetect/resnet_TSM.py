...
#         self.fc1 = nn.Linear(512 * block.expansion, num_classes)                   
    ...
    def flow_computation(self, x, pos=2, temperature=100):
        ...
        x_pre, x_post = self.extract_temporal_features(x)
        match = self.compute_matching(x_pre, x_post)
        flow, confidence = self.match_to_flow_soft(match, k, x.size(3), x.size(4), temperature)

        return flow, confidence

    def channel_reduction(self, x):
        # Assuming you want to apply a 1x1 convolution followed by ReLU
        reduction_conv = torch.nn.Conv2d(x.size(1), new_channel_size, kernel_size=1)
        x = F.relu(reduction_conv(x))
        return x
    ...
    def extract_temporal_features(self, x):
        x_pre = x[:, :, :-1].permute(0, 2, 1, 3, 4).contiguous().view(-1, x.size(1), x.size(3), x.size(4))
        x_post = x[:, :, 1:].permute(0, 2, 1, 3, 4).contiguous().view(-1, x.size(1), x.size(3), x.size(4))

        return x_pre, x_post

    def compute_matching(self, x_pre, x_post):
        return self.matching_layer(x_pre, x_post)

    def match_to_flow_soft(self, match, k, h, w, temperature):
        # Assume match is a tensor of shape (batch_size * T, H*W, H*W)

        match = match.view(-1, k, h * w)  # Reshape for softmax
        softmax_match = F.softmax(match / temperature, dim=2)  # Apply softmax along the last dimension

        # Extract u, v, and confidence from the softmax_match tensor
        u = softmax_match[:, :k // 2, :].view(-1, k // 2, h, w)
        v = softmax_match[:, k // 2:, :].view(-1, k // 2, h, w)

        # Calculate confidence as the average of softmax scores for u and v
        confidence = (u.mean(dim=(1, 2, 3)) + v.mean(dim=(1, 2, 3))) / 2.0
